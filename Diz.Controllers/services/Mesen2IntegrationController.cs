...
namespace Diz.Controllers.services
{
    ...
    public class Mesen2IntegrationController : IMesen2IntegrationController, IDisposable
    {
        ...
        public void ShowConnectionDialog()
        {
            ...
            try
            {
                // Create the WinForms connection dialog
                using var connectionDialog = new Diz.Ui.Winforms.dialogs.Mesen2ConnectionDialog(_configuration);
                
                // Show the dialog
                if (connectionDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    _gui?.ShowInfo("Mesen2 Integration", "Connection configuration updated successfully.");
                }
            }
            catch (Exception ex)
            {
        ...
        public void ShowStatusWindow()
        {
            ...
            try
            {
                // Create the WinForms status dialog
                using var statusDialog = new Diz.Ui.Winforms.dialogs.Mesen2StatusDialog(this);
        ...
        public void ShowTraceViewer()
        {
            ...
            try
            {
                // Create the WinForms trace viewer dialog
                var traceViewer = new Diz.Ui.Winforms.dialogs.Mesen2TraceViewerDialog(this);
                traceViewer.Show(); // Use Show() instead of ShowDialog() to make it modeless
            }
            catch (Exception ex)
            {
        ...
        public void ShowDashboard()
        {
            ...
            try
            {
                // Create the WinForms dashboard dialog
                var dashboard = new Diz.Ui.Winforms.dialogs.Mesen2DashboardDialog(this);
                dashboard.Show(); // Use Show() to make it modeless for convenient access
            }
            catch (Exception ex)
            {
        ...
        public void ShowAdvancedConfigurationDialog()
        {
            ...
            try
            {
                // Create the WinForms advanced configuration dialog
                using var configDialog = new Diz.Ui.Winforms.dialogs.Mesen2ConfigurationDialog(_configuration);
                
                // Show the dialog
                if (configDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    _gui?.ShowInfo("Mesen2 Integration", "Advanced configuration updated successfully.");
                }
            }
            catch (Exception ex)
            {
